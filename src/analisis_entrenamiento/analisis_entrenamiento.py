...
# -----------------------------
file_path = 'src/results.csv'  # Asegúrate de que tu archivo se llame así o cambia esta ruta
...
for i, (train_col, val_col, title) in enumerate(loss_metrics):
    ax = axes_loss[i]
    ax.plot(data['epoch'], data[train_col], label='Entrenamiento', linewidth=2)
    ax.plot(data['epoch'], data[val_col], label='Validación', linewidth=2, linestyle='--')
    ax.set_title(title)
...
plt.tight_layout()
plt.savefig('grafico_perdidas.png') # Guarda la imagen
...
ax1 = axes_metrics[0]
ax1.plot(data['epoch'], data['metrics/mAP50(B)'], label='mAP@50 (Holgado)', color='blue')
ax1.plot(data['epoch'], data['metrics/mAP50-95(B)'], label='mAP@50-95 (Estricto)', color='green', linewidth=2.5)
...
ax2 = axes_metrics[1]
ax2.plot(data['epoch'], data['metrics/precision(B)'], label='Precisión (Calidad)', color='purple', alpha=0.7)
ax2.plot(data['epoch'], data['metrics/recall(B)'], label='Recall (Cantidad)', color='orange', alpha=0.7)
...
plt.tight_layout()
plt.savefig('grafico_metricas.png') # Guarda la imagen
